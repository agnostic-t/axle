...
static int axle_compile_sources(const axle_receipt *receipt,
                                const char *base_path, bool rebuild,
                                bool silent);

static int axle_link_executable(const axle_receipt *receipt,
                                const char *base_path, bool silent);
...
int axle_build(const axle_receipt *receipt, const char *base_path,
               bool hide_greeting, bool rebuild, bool silent) {

  if (!hide_greeting && !silent)
    printf("[axle] version " AXLE_VERSION "\n");

  if (!silent) {
    printf("[axle] building %s project, version: %s\n", receipt->metadata.name,
           receipt->metadata.version);
  }

  if (axle_hooks_run(receipt->hooks.pre_build, "pre-build", receipt, base_path,
                     rebuild, silent) < 0) {
    fprintf(stderr, "%s[axle] pre-build hook failed%s\n", xfore.red,
            xfore.normal);
    return -1;
  }

  /* An only-deps receipt may still have hooks. This is useful for wrapper
   * modules that invoke an external build system and then expose the
   * generated include/lib directories to downstream modules. */
  if (receipt->only_deps) {
    if (!silent)
      printf("[axle][compilation] only deps specified, %sskipping native "
             "compilation/linking%s\n",
  ...
  if (!receipt->output.obj_path || !receipt->output.path) {
    fprintf(stderr,
            "%s[axle] non-only-deps project has no object/output path%s\n",
  ...
  if (stat(obj_path, &st) != 0) {
    ...
    if (status != 0) {
      fprintf(stderr,
              "%s[axle] failed to build project%s, cannot create object files "
              "(objs) directory: %s\n",
  ...
  if (stat(out_path, &st) != 0) {
    ...
    if (status != 0) {
      fprintf(stderr,
              "%s[axle] failed to build project%s, cannot create output files "
              "(bin) directory: %s\n",
  ...
  if (ret < 0) {
    fprintf(stderr,
            "%s[axle] failed to build project%s at source compilation stage, "
            "code: %d\n",
  ...
  if (ret < 0) {
    fprintf(stderr,
            "%s[axle] failed to build project%s at linking stage, code: %d\n",
            xfore.red, xfore.normal, ret);
    return -1;
  }

post_build:
  if (axle_hooks_run(receipt->hooks.post_build, "post-build", receipt,
                     base_path, rebuild, silent) < 0) {
    fprintf(stderr, "%s[axle] post-build hook failed%s\n", xfore.red,
            xfore.normal);
    return -1;
  }
...
static int axle_compile_sources(const axle_receipt *receipt,
  ...
  if (!sources->sources && !receipt->only_deps) {
    ...
    printf(
        "[axle][compilation] only deps specified, %sskipping compilation%s\n",
        xfore.magenta, xfore.normal);
    return 0;
  }
  ...
  char *_cmb_includes = sources->incl_dirs
                            ? uax_concat((const char **)out_incls, " ", "-I")
                            : NULL;

  if (incls_sz != 0)
    uax_free_strlist(&out_incls, &incls_sz);

  char *_cmb_pkgs_cflags =
      sources->pkgs ? uax_concat(sources->pkgs, ") ", " $(pkg-config --cflags ")
                    : NULL;

  char *_cmb_defines = receipt->target.defines
                           ? uax_concat(receipt->target.defines, " ", " -D")
                           : NULL;
  ...
  for (size_t i = 0; i < src_sz; i++) {
    ...
    if (nr_ret == -1) {
      fprintf(stderr,
              "%s[axle][compilation] failed to get information%s about files\n",
              xfore.red, xfore.normal);
      free(cropped);
      free(changed);
      free(o_path);
    ...
    if (0 != ret) {
      fprintf(stderr,
              "%s[axle][compilation] failed to execute compilation command%s. "
              "Exit code: %d\n",
              xfore.red, xfore.normal, ret);
...
static _axle_link_metadata axle_link_metadata(const axle_receipt *receipt,
  ...
  char *_cmb_libdirs =
      sources->lib_dirs ? uax_concat((const char **)out_libs, " ", "-L") : NULL;

  if (out_libs)
    uax_free_strlist(&out_libs, &libs_sz);

  char *_cmb_libs = sources->libs ? uax_concat(sources->libs, " ", "-l") : NULL;

  char *_cmb_pkgs_libs =
      sources->pkgs ? uax_concat(sources->pkgs, ") ", " $(pkg-config --libs ")
                    : NULL;

  return (_axle_link_metadata){._cmb_libdirs = _cmb_libdirs,
                               ._cmb_libs = _cmb_libs,
...
static int axle_link_executable(const axle_receipt *receipt,
  ...
  if (md._cmb_pkgs_libs) {
    uax_ip_strextend(&cmd, md._cmb_pkgs_libs);
    uax_ip_strextend(&cmd, " ");
  }

  if (md._cmb_pkgs_libs)
    free(md._cmb_pkgs_libs);
  if (md._cmb_libdirs)
    free(md._cmb_libdirs);
  if (md._cmb_libs)
    free(md._cmb_libs);
  ...
  if (ret != 0) {
    fprintf(stderr,
            "%s[axle][linking] failed to execute linking%s command. Exit code: "
            "%d\n",
            xfore.red, xfore.normal, ret);
    return -1;
  }
